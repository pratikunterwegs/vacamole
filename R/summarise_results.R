...
summarise_results <- function(seir_output, params, t_vec) {
  ...
  # calculate cases ------------------------------------------------------------
  new_cases <- sweep(
    params$sigma * (seir_output$E + seir_output$Ev_1d +
  ...
  new_deaths <-
    ...
    sweep(
      seir_output$H + seir_output$Hv_1d + seir_output$Hv_2d +
        seir_output$Hv_3d + seir_output$Hv_4d + seir_output$Hv_5d, 2, params$d,
      "*"
    ) +
    # from IC compartment
    sweep(
      seir_output$IC + seir_output$ICv_1d + seir_output$ICv_2d +
        seir_output$ICv_3d + seir_output$ICv_4d + seir_output$ICv_5d, 2,
      params$d_ic, "*"
    ) +
    # from hospital after IC
    sweep(
      seir_output$H_IC + seir_output$H_ICv_1d + seir_output$H_ICv_2d +
  ...
  rtn <- dplyr::bind_rows(
    new_infections, new_cases, hosp_admissions, ic_admissions,
    new_deaths
  ) %>%
    tidyr::pivot_longer(
      cols = age_group1:age_group9,
      names_to = "age_group",
