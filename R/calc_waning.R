#' @title Calculate vaccine efficacy waning
#'
#' @description Based on the weighted overage of the VE, daily vaccination rate,
...
calc_waning <- function(prop, time_points, k, t0) {
  ...
  # proportion of people vaccinated within time point 0 and the current time
  # point
  for (t in 1:length(time_points)) {
    if (is.na(time_points[t])) {
      # go to next time point if t = NaN
      # (because vaccination hasn't started yet)
      next
    }
