#' @title Convert cumulative vaccination schedule to non-cumulative
#'
...
#' @param ve_pars a data frame with VE estimates by vaccine product, dose,
...
#'  waning occurs)
#' @importFrom dplyr select mutate mutate_at filter add_row group_by summarise
#' matches slice
...
convert_vac_schedule <- function(vac_schedule,
  ...
  names_check <- stringr::str_extract(names(vac_schedule),
  ...
  if (lubridate::is.Date(vac_schedule$date)) {
    date_vec <- vac_schedule$date
  } else {
  ...
  if (num_age_groups == 10) {
    vac_schedule <- vac_schedule %>%
      ...
      select(
        ...
        -.data$pf_d1_10, -.data$pf_d2_10,
        -.data$mo_d1_10, -.data$mo_d2_10,
  ...
  vac_schedule_rate <- vac_schedule_daily / (1 - stats::lag(vac_schedule[, -1]))
  ...
  vac_rates_long <- vac_schedule_rate %>%
    pivot_longer(
      cols = !date, names_to = c("vac_product", "dose", "age_group"),
  ...
  vac_prop <- left_join(
    vac_schedule_cs, vac_prop_total,
    by = c("date", "dose", "age_group")
  ) %>%
  ...
  vac_info_joined <- left_join(
    vac_rates_long, vac_prop,
    by = c("date", "vac_product", "dose", "age_group")
  ) %>%
  ...
  if (wane) {
    ...
    ve_dat1 <- left_join(
      vac_info_joined, first_day_vac,
      by = "dose"
    ) %>%
      mutate(
        time_since_vac_start = ifelse(
          date >= first_day, yes = date - first_day + 1, no = NA
        )
      ) %>%
      group_by(vac_product, dose, age_group) %>%
      group_modify(~ calc_waning(
        prop = .x$vac_prop, time_point = .x$time_since_vac_start,
        k = k_inf, t0 = t0
      )) %>%
      mutate(
        date = row_number() + vac_info_joined$date[1] - 1
      ) %>% # convert t to date
    ...
    ve_dat <- left_join(
      ve_dat1, ve_dat2,
      by = c("vac_product", "dose", "age_group", "t", "date")
    ) %>%
      left_join(
        ., ve_pars,
        by = c("vac_product", "dose", "age_group")
      ) %>%
      left_join(
        ., vac_info_joined,
        by = c("date", "vac_product", "dose", "age_group")
      ...
      mutate(
        ve_wane_inf = ifelse(ve - w_inf < 0, 0, ve - w_inf),
    ...
    rtn <- ve_comp_eta %>%
      filter(
        !(outcome == "hospitalisation" & param == "eta_inf"),
      ...
      mutate(
        param = ifelse(
          (param == "comp_ve_inf" | param == "comp_ve_sev"),
          yes = "comp_ve", no = param
        ),
        param = ifelse(
          (param == "eta_inf" | param == "eta_sev"),
          yes = "eta", no = param
        ),
      ) %>%
      ungroup()
    ...
    ve_comp <- ve_dat %>%
      ...
      # comp_ve should be > 0
      mutate(
        comp_ve = ifelse(comp_ve == 0, NA, comp_ve),
        comp_delay = ifelse(comp_ve == 0, NA, comp_delay)
      ) %>%
      fill(comp_ve, .direction = c("down")) %>%
      fill(comp_delay, .direction = c("down")) %>%
      mutate(
        eta = 1 - comp_ve,
